...
class BronzeStore:
    ...
    def last_successful_end(self, source: str) -> date | None:
        row = self.conn.execute(
            "SELECT max(date_end) FROM bronze.collection_runs WHERE source = %s AND status = 'success'",
            (source,),
        ).fetchone()
    ...
    def start_run(self, source: str, date_start: date, date_end: date) -> int:
        run_id = self.conn.execute(
            "INSERT INTO bronze.collection_runs (source, date_start, date_end) VALUES (%s, %s, %s) RETURNING run_id",
    ...
    def save(self, source: str, external_id: str, payload: dict[str, Any], run_id: int) -> str:
        ...
        if row[0] != new_hash:
            ...
            return "changed"
        self.conn.execute(
            "UPDATE bronze.raw_documents SET last_seen_run = %s WHERE source = %s AND external_id = %s",
